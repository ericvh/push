...
# number of cmd element keep track of symbol use to make sure each node in our digraph is unique.
ncel := array[256] of { * => 0};
EnumNode: adt {
	op: int;
	n: int; # my id
	left: cyclic ref EnumNode;
	right: cyclic ref EnumNode;
	
	cmd2enum: fn(c: ref Cmd): ref EnumNode;
	tree: fn(t: self ref EnumNode): string;
};

EnumNode.cmd2enum(c: ref Cmd): ref EnumNode
{
	if(c == nil)
		return nil;
	return ref EnumNode(c.op, ncel[c.op]++, EnumNode.cmd2enum(c.left), EnumNode.cmd2enum(c.right));
}

Cmd.tree(c: self ref Cmd): string
{
	return EnumNode.cmd2enum(c).tree();
}

EnumNode.tree(t: self ref EnumNode): string
{
	if (t == nil)
		return "";
	case t.op {
	Csimple to Cprivate =>
		s: string;
		if(t.left != nil){
			s += sys->sprint("\"%s%d\" -> \"%s%d\";\n", opcodes[t.op], t.n, opcodes[t.left.op], t.left.n);
			s += t.left.tree();
		}
		if(t.right != nil){
			s += sys->sprint("\"%s%d\" -> \"%s%d\";\n", opcodes[t.op], t.n, opcodes[t.right.op], t.right.n);
			s += t.right.tree();
		}
		return s;
	* =>
		if (t.op >= Cprivate)
			return "Priv+" + string (t.op - Cprivate);
		else
			return "?" + string t.op;
	}
	return nil;
