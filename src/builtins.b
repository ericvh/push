implement Pushbuiltin;

#
#	"builtins" builtin, defines:
#
#	env	- print environment or individual elements
#	eval	- interpret arguments as push input
#	exit	- exit toplevel, eval or subshell
#	load	- load a builtin
#	prompt	- print or set prompt
#	quote	- print arguments quoted as input for push
#	run	- interpret a file as push input
...
Cmd, Env, Stab:	import pushlib;
sys, bufio:	import pushlib;
...
#	Install commands.
#
pushinit(nil: list of string, lib: Pushlib, this: Pushbuiltin, e: ref Env)
{
	pushlib = lib;
	e.defbuiltin("dargs", this);
...
#	Execute a builtin.
#
pushcmd(e: ref Env, l: list of string)
{
	case hd l {
	"env" =>
		l = tl l;
...
#
#	Catenate arguments and interpret as push input.
#
eval(e: ref Env, l: list of string)
{
	...
	e = e.copy();
	e.flags &= ~pushlib->EInter;
	e.sopen(s);
	pushlib->parse->parse(e);
}

#
#	Interpret file as push input.
#
run(e: ref Env, l: list of string): int
{
	...
	while (s[0] == '-') {
		if (s == "-")
			...
			for (i := 1; i < len s; i++) {
				case s[i] {
				'd' =>
					e.flags |= pushlib->EDumping;
				'e' =>
					e.flags |= pushlib->ERaise;
				'n' =>
					e.flags |= pushlib->ENoxeq;
				'x' =>
					e.flags |= pushlib->EEcho;
				* =>
					return 0;
				}
			}
	...
	if (fd == nil) {
		err := pushlib->errstr();
		if (pushlib->nonexistent(err) && s[0] != '/' && s[0:2] != "./") {
			fd = sys->open(pushlib->LIB + s, Sys->OREAD);
			if (fd == nil)
				err = pushlib->errstr();
			else
				s = pushlib->LIB + s;
	...
	e.local = Stab.new();
	e.local.assign(pushlib->ARGS, tl l);
	e.flags &= ~pushlib->EInter;
	e.fopen(fd, s);
	pushlib->parse->parse(e);
	return 1;
