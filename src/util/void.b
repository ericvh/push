...
mainfn(in: ref Iobuf)
{
	# file2chan that returns what I want. will I be able to to do this from a device? 
	
	# so the user writes to the device. 
	# we return an fd to a push instance. 
	# perfect. 
	
	# need to add shutdown code to this. 
	
	ctlio := sys->file2chan("/chan", "void");
	if(ctlio == nil)
		fail("fail", sys->sprint("can't create /chan/void: %r"));
	...
	(nil, nil, fid, rc) := <-ctlio.read =>
		...
		# that is perfect. I get that so what are we sending? 
		alt{
...
newinstance()
{
	c := load Command "/dis/os.dis";
	# need a way of starting a push server on this. 
	c->init(nil, "os", "emu");
	# I need it to return what the system host and port is. 
	# this is where the reglisten should happen.
	# come to think of it this should really be a shell script. 
}

addreg(): ref Registered
{
