...
#
Stab.update(t: self ref Stab, s: string, tag: int, v: list of string, f: ref Cmd, b: Pushbuiltin): ref Symb
{
	if (t.copy) {
...
#	Define a builtin.
#
Stab.defbuiltin(t: self ref Stab, s: string, b: Pushbuiltin)
{
...
Env.new(): ref Env
{
	return ref Env(Stab.new(), nil, ETop, nil, nil, nil, 8, nil, nil, nil, nil, 0);
}
...
#	Lookup builtin.
#
Env.builtin(e: self ref Env, s: string): Pushbuiltin
{
	v := e.global.find(s);
	if (v == nil)
		return nil;
...
#	Define a builtin.
#
Env.defbuiltin(e: self ref Env, s: string, b: Pushbuiltin)
{
