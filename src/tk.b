...
Env, Stab, Symb:	import pushlib;
sys, bufio, tk:		import pushlib;
gtop, gctxt, ident:	import pushlib;
...
{
	if (e != nil && (e.flags & pushlib->EDumping))
...
#	Serve loop.
#
tkserve(push: chan of string)
{
	pushlib->reap();
	for (;;) {
		cmd := <-push;
		if (pushlib->servechan != nil && len cmd > 1) {
			cmd[len cmd - 1] = '\n';
			pushlib->servechan <-= array of byte cmd[1:];
		}
	}
}
...
{
	return tk->quote("send push " + tk->quote(c));
}
...
layout(e: ref Env, l: list of string)
{
	...
	if (!tkserving) {
		tkserving = 1;
		push := chan of string;
		tk->namechan(gtop, push, "push");
		spawn tkserve(push);
		pushlib->startserve = 1;
	}
}

dumpbutton(out: ref Iobuf, w: string, s: ref Symb)
{
	out.puts(sys->sprint("tk def %s %s %s", w, s.name, pushlib->quote(hd s.value)));
	if (s.tag == Cibutton)
		out.puts(sys->sprint(" %s", pushlib->quote(hd tl s.value)));
	out.puts(";\n");
}
