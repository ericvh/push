...
#	Install commands.
#
pushinit(nil: list of string, lib: Pushlib, nil: Pushbuiltin, e: ref Env)
{
	pushlib = lib;
	if (pushlib->histchan != nil)
		return;
	...
	seek = 0;
	(f, c) := e.servefile(pushlib->HISTF);
	spawn servehist(f, c);
	(f, c) = e.servefile(pushlib->PUSHF);
	spawn servepush(f, c);
}

pushcmd(nil: ref Env, nil: list of string)
{
}
...
loadhist(data: array of byte, fid: int, wc: Sys->Rwrite, c: ref Sys->FileIO)
{
	...
	if (chanfill != nil)
		in = chanfill->init(data, fid, wc, c, pushlib->bufio);
	if (in == nil) {
...
servepush(f: string, c: ref Sys->FileIO)
{
	pushlib->reap();
	for (;;) {
		alt {
		...
		(off, data, fid, wc) := <-c.write =>
			if (wc != nil) {
				wc <-= (len data, nil);
				if (pushlib->servechan != nil && len data > 0)
					pushlib->servechan <-= data;
			}
		}
	}
}
