...
tail:	for (;;) { 
	...
	Cfanout =>
		do {
			# get a pipe
			outfds := e.mp;
			fds :=  e.multipipe(e.npipe);
			if(e.flags & EPdebug)
				sys->print("oufds %d fds %d\n", len outfds, len fds);
			if (fds == nil)
				return;
			if(e.mp == nil)
				e.couldnot("find","multipipe");
			n := e.clone();
			ffds := e.pipe();
			...
			if(e.flags & EPdebug)
				for(i = 0; i < len fds; i++)
					for(j := 0; j < len fds[0]; j++)
						sys->print("fanout fds[%d][%d] %bx\n", i, j, sys->fstat(fds[i][j]).t1.qid.path);
			if(e.flags & EPdebug)
				for(i = 0; i < len outfds; i++)
					for(j = 0; j < len outfds[0]; j++)
						sys->print("fanout outfds[%d][%d] %bx\n", i, j, sys->fstat(outfds[i][j]).t1.qid.path);						
			c = c.right;
		} while (c.op == Cfanout);
		continue tail;
	Cfanin =>
		do {
			...
			if(fds == nil)
				return;
			# going to need some logic for the implicit ones at the start
			e.mp = e.multipipe(e.npipe);
			if(e.flags & EPdebug)
				for(i := 0; i < len e.mp; i++)
					for(j := 0; j < len e.mp[0]; j++)
						sys->print("fanin e.mp[%d][%d] %bx\n", i, j, sys->fstat(e.mp[i][j]).t1.qid.path);
...
{
	pid := sys->pctl(0, nil);
	in := bufio->open("/prog/"+string pid+"/fd", bufio->OREAD);
	for(i := 0; i < 4; i++){
		if((s := in.gets('\n')) == nil)
			raise "fail: bad fd file at /prog/"+string pid+"/fd";
		sys->print("%s", s);
	}
}
printdotdigraph(c:  ref Cmd, e: ref Env)
{
	f := e.outfile();
	f.puts("digraph pushtree {\n");
	f.puts("	size=\"6,6\";\n");
	f.puts(c.tree());
	f.puts("}\n");
	f.close();
