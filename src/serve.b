...
#
#	Serve push console reads.  Favours other programs
...
outer:	for (;;) {
		...
		(off, count, fid, rc) := <-c.read =>
			fid = fid; # shut up warning
			off = off; # shut up warning
			count = count; # shut up warning
			if (rc == nil)
				break;
...
#
#	Begin serving the push console.
#
